...
class Loader(object):
    ...
    def _load_data(self):
        if self.data_file_name.endswith('csv'):
            df = pd.read_csv(self.data_file_name)
        elif self.data_file_name.endswith('json'):
            df = pd.read_json(self.data_file_name, lines=True)
        else:
    ...
    def _get_tokenizer(self):
        ...
        if self.args.encoder == 'm-bert':
            self.tokenizer = BertTokenizer.from_pretrained('bert-base-multilingual-uncased',truncation=True)#khowell bert team recommends cased over bert-base-multilingual-uncased
        elif self.args.encoder in ('distill-bert', 'distill-bert-original'):
            ...
            self.tokenizer = XLMRobertaTokenizer.from_pretrained('xlm-roberta-large',truncation=True)#khowell other option is xlm-roberta-large
    ...
    def prepare(self, minibatch_data):
        ...
        if self.label == 'hard':
            label_tensor = np.array([label for _, label in minibatch_data], dtype=np.int64)
        ...
        if self.args.encoder in ('bert', 'roberta', 'albert', 'distill-bert', 'distill-bert-original', 'm-bert', 'xlm-roberta'):
